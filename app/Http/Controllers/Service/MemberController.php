...
class MemberController extends Controller
{
    public function register(Request $request)
    {
       //后台验证
        ...
        $result = Member::where('email','=',$email)->get();
        //$result2 = Member::where('name','=',$nickname)->get();
        if(!$result->isEmpty() ){
                $m3_result->status = 9;
                $m3_result->message = '邮箱被使用';
                return $m3_result->toJson();
        }
        ...
        $m3_email->subject = 'zmmApp验证';
        $m3_email->content = '请于24小时点击该链接完成验证. http://zmm.esy.es/service/validate_email'
            . "?member_id=$member->id&code=$uuid";
        $m3_result->message = $m3_email->content;
