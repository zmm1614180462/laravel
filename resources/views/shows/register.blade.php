...
@section('wrapper')
    ...
    <div class="aw-register-box">
        ...
        <div class="mod-body">
            <form class="aw-register-form" action="false" method="post" id="register_form">
                ...
                <ul>
                    ...
                    <li>
                        <input id="registe_name" class="aw-register-name form-control" reg="^[a-zA-z]\w{3,15}$" type="text" name="user_name" placeholder="账号" state="0"  value="">
                        <span style="display: none" class="aw-reg-tips aw-reg-err"><i class="aw-icon i-err"></i>请输入4-12位账号</span>
                    </li>
                    <li>
                        <input id="registe_email" class="aw-register-email form-control" reg="^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+" type="text" placeholder="邮箱" name="email" state="0" value="">
                        <span style="display: none" class="aw-reg-tips aw-reg-err"><i class="aw-icon i-err"></i>邮箱格式不正确</span>
                    </li>
                    <li>
                        <input id="registe_password" class="aw-register-pwd form-control"  reg="^[A-Za-z0-9]{6,20}$" type="password" name="password" placeholder="密码" state="0">
                        <span style="display: none" class="aw-reg-tips aw-reg-err"><i class="aw-icon i-err"></i>密码6-20位字母数字</span>
                    </li>
                    <li>
                        <input id="again_password" class="aw-register-pwd form-control" type="password" name="again_password" placeholder="密码" state="0" >
                        <span style="display: none" class="aw-reg-tips aw-reg-err"><i class="aw-icon i-err"></i>两次密码不一致</span>
                    </li>
                    ...
                    <li class="aw-register-verify">
                            <img width="100" class="pull-right"  onclick="this.src =  '/service/validate_code/create?' + Math.floor(Math.random() * 10000);" src="/service/validate_code/create">

                        <input id="seccode_verify" type="text" class="form-control" name="seccode_verify" placeholder="验证码">
                    </li>
                    <li class="last">
                    ...
                    <li class="clearfix">
                        <button id="register" class="btn btn-large btn-blue btn-block" >注册</button>
                    </li>
                </ul>
            </form>
        </div>
    ...
    <script>
        $('#register_form input').blur(function () {
            ...
            if(name =='again_password'){
                if($(this).val()!=$('#registe_password').val()){
                    $(this).siblings('span').css('display','block');
                    $(this).attr('state','0');
                }
            }
        ...
        $('#register').click(function (ev) {
           ev.preventDefault();
            var name = $('#registe_name').attr('state');
            var email = $('#registe_email').attr('state');
            var password = $('#registe_password').attr('state');
            var again = $('#again_password').attr('state');

            //获取要提交的值
            var nameVal = $('#registe_name').val();
            var emailVal = $('#registe_email').val();
            var passwordVal = $('#registe_password').val();
            var confirm = $('#again_password').val();
            var seccode_verify = $('#seccode_verify').val();
            if(name==0 || email==0 || password==0 || again==0){
                console.log('错误');
            }else{
                $.ajax({
                    ...
                    type:'post',
                    dataType:'json',
                    data:{name:nameVal,email:emailVal,password:passwordVal,confirm:confirm,validate_code:seccode_verify,_token: "{{csrf_token()}}"},
